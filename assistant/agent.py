...
async def _ask_once(
    ...
    try:
        ...
        # Это было слепое пятно: 17 сентября разбирали, почему на шеринг приходит
        # «Записала.» без чистовика, и доказать упор в лимит было нечем — сюда не
        # писалось ничего. Теперь видно в логах Railway по слову «лимит заходов».
        logger.warning(
            "лимит заходов в инструменты исчерпан (%d) — добираю финальный текст "
            "без инструментов; ответ может выйти коротким",
            MAX_TOOL_ROUNDS,
        )
...
# ── Женский род ─────────────────────────────────────────────────────────
# Анджелина — она. В характере это написано прямым текстом, но модель не
# держит: в коротких служебных ответах сваливается в «Понял», «Записал».
# Проверено 17-18 августа, уже после того как правило добавили в промпт.
# Тот же вывод, что был со скобкой времени: важное поведение делаем кодом.
_FEM = {
    "понял": "поняла", "записал": "записала", "услышал": "услышала",
    "увидел": "увидела", "заметил": "заметила", "проверил": "проверила",
    "посмотрел": "посмотрела", "сохранил": "сохранила", "добавил": "добавила",
    "поставил": "поставила", "отправил": "отправила", "удалил": "удалила",
    "перенёс": "перенесла", "нашёл": "нашла", "сделал": "сделала",
    "подумал": "подумала", "решил": "решила", "забыл": "забыла",
    "смог": "смогла", "хотел": "хотела", "спросил": "спросила",
    "готов": "готова", "рад": "рада", "уверен": "уверена",
    "согласен": "согласна", "прав": "права", "должен": "должна",
}
# Два безопасных случая: слово открывает строку («Записал. Обновляю…»)
# и связка «я + глагол» («я записал»). Всё остальное не трогаем — там
# запросто может быть речь про Бена или сыновей.
_FEM_LINE = re.compile(r"^(%s)\b" % "|".join(_FEM), re.IGNORECASE | re.MULTILINE)
_FEM_YA = re.compile(r"\b([Яя]\s+)(%s)\b" % "|".join(_FEM), re.IGNORECASE)
# Третий случай, встретился живьём: «День 5, конец менструации записал.» —
# глагол в самом конце фразы, без «я» и не в начале строки.
_FEM_TAIL = re.compile(r"\b(%s)(?=\s*[.!?]|$)" % "|".join(_FEM), re.IGNORECASE)
# ...но только если в строке не говорится о ком-то ещё. Иначе «Кирилл всё
# записал» превратится в «записала». Признаки чужого подлежащего: он/они
# и любое имя с заглавной буквы не в начале строки.
_SOMEONE_ELSE = re.compile(
    r"\b([Оо]н|[Оо]ни|[Ее]го|[Ее]му|[Ии]м|[Ее]ё|[Нн]её|муж|сын|Бен|Бэн|Кирилл|Чарли)\b"
    r"|(?<!^)(?<![.!?]\s)\b[А-ЯЁ][а-яё]{2,}",
)


def _same_case(src_word: str, repl: str) -> str:
    """Сохранить заглавную букву: «Записал» → «Записала»."""
    return repl.capitalize() if src_word[:1].isupper() else repl


def _feminize(text: str) -> str:
    """Привести речь о себе к женскому роду.

    Строки с кавычками пропускаем целиком: там она цитирует Катю или
    чужие слова, и «он сказал: понял» править нельзя.
    """
    out = []
    for line in text.split("\n"):
        if any(q in line for q in ('"', "«", "„", "”")):
            out.append(line)
            continue
        line = _FEM_LINE.sub(lambda m: _same_case(m.group(1), _FEM[m.group(1).lower()]), line)
        line = _FEM_YA.sub(
            lambda m: m.group(1) + _same_case(m.group(2), _FEM[m.group(2).lower()]), line
        )
        if not _SOMEONE_ELSE.search(line):
            line = _FEM_TAIL.sub(
                lambda m: _same_case(m.group(1), _FEM[m.group(1).lower()]), line
            )
        out.append(line)
    return "\n".join(out)
